/**
 * Ohara — OpenCode plugin adapter
 *
 * Thin layer that connects OpenCode's event system to the Ohara Go binary.
 * The Go binary runs as a local HTTP server and handles all persistence.
 *
 * Flow:
 *   OpenCode events → this plugin → HTTP calls → ohara serve → SQLite
 *
 * Session resilience:
...
const MEMORY_INSTRUCTIONS = `## Ohara Persistent Memory — Protocol

You have access to Ohara, a persistent memory system that survives across sessions and compactions.
...
async function oharaFetch(
  path: string,
  opts: { method?: string; body?: any } = {}
): Promise<any> {
  try {
    const res = await fetch(`${OHARA_URL}${path}`, {
...
async function isOharaRunning(): Promise<boolean> {
  try {
    const res = await fetch(`${OHARA_URL}/health`, {
...
/**
 * Strip <private>...</private> tags before sending to Ohara.
...
export const Ohara: Plugin = async (ctx) => {
  ...
  // Track which sessions we've already ensured exist in ohara
  ...
  // We must not register them as top-level Ohara sessions — they cause session
  ...
  async function ensureSession(sessionId: string): Promise<void> {
    ...
    // Do not register sub-agent sessions in Ohara (issue #116).
  ...
  // Try to start ohara server if not running
  const running = await isOharaRunning()
  if (!running) {
    try {
      Bun.spawn([OHARA_BIN, "serve"], {
        stdout: "ignore",
        stderr: "ignore",
        stdin: "ignore",
      })
  ...
  if (oldProject !== project) {
    await oharaFetch("/projects/migrate", {
      method: "POST",
      body: { old_project: oldProject, new_project: project },
    })
  }

  // Auto-import: if .ohara/manifest.json exists in the project repo,
  // run `ohara sync --import` to load any new chunks into the local DB.
  ...
  try {
    const manifestFile = `${ctx.directory}/.ohara/manifest.json`
    const file = Bun.file(manifestFile)
    if (await file.exists()) {
      Bun.spawn([OHARA_BIN, "sync", "--import"], {
        cwd: ctx.directory,
        stdout: "ignore",
        stderr: "ignore",
        stdin: "ignore",
  ...
  return {
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        // top-level Ohara sessions. They cause massive session inflation
    ...
    "experimental.session.compacting": async (input, output) => {
      ...
      // compacted summary to Ohara. The new agent reads the compacted
      // summary and this instruction, then saves it as a session summary.
      output.context.push(
        `CRITICAL INSTRUCTION FOR COMPACTED SUMMARY:\n` +
        `The agent has access to Ohara persistent memory via MCP tools.\n` +
