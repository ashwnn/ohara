...
[claude-mem](https://github.com/thedotmack/claude-mem) is a great project (28K+ stars!) that inspired Ohara. But we made fundamentally different design decisions:
...
| **Database** | Single `~/.ohara/ohara.db` file | SQLite + ChromaDB (two storage systems) |
...
**Ohara** lets the agent decide what's worth remembering. The agent already has the LLM, the context, and understands what just happened. Why run a separate compression pipeline?
...
- [Installation](INSTALLATION.md) — get Ohara running
