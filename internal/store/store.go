...
func (s *Store) migrate() error {
	...
	if _, err := s.execHook(s.db, schemaVersionTable); err != nil {
	...
	if _, err := s.execHook(s.db, syncMutationsIndices1); err != nil {
		return err
	}
	...
	if _, err := s.execHook(s.db, syncMutationsIndices2); err != nil {
		return err
	}
	...
	if err == sql.ErrNoRows {
		promptTriggers := promptsFTSTriggers
	...
	if _, err := s.execHook(s.db, memoryItemsSchema); err != nil {
		return err
	}
	...
	if err == sql.ErrNoRows {
		if _, err := s.execHook(s.db, memoryItemsFTSTriggers); err != nil {
			return err
		}
	}
