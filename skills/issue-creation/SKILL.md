---
name: ohara-issue-creation
description: >
  Issue creation workflow for Ohara following the issue-first enforcement system.
...
| **Ohara Version** | Output of `ohara version` |
...
### Problem Description
When scripting with ohara, parsing the human-readable output of mem search is fragile. There's no machine-readable output format.

### Proposed Solution
Add a \`--json\` flag to \`ohara mem search\` that outputs results as JSON.

Example:
\`\`\`bash
ohara mem search \"auth middleware\" --json
\`\`\`
